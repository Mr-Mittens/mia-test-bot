# dates x events x notes handler
...
def _start(message):
    ...
    ## send first msg
    msg = "Hey there "+str(message.chat.username)+\
          ", I'm your personal assistant. let me handle your events and notes for you. To learn how to use me, use \n/help"
...
def _help(message):
    msg = "Set an event, like someone's birthday, for example: \n\
            sofia : Oct 25 \nAnd I'm gonna save the date and every Oct 25 I will remind you of sofia's birthday. \n\
Save a date with \n/save \n\
...
def chat(message):
    ...
    if any(x in txt.lower() for x in ["thank","thx","cool"]):
        ...
        msg = "hey" if str(message.chat.username) == "none" else "hey "+str(message.chat.username)
    else:
...
def scheduler():
    ...
    for user in lst_users:
        #res = requests.get("https://api.telegram.org/bot1494658770:"+telegram_key+"/sendMessage?chat_id="+user+"&text=heyhoney")
...
if config.ENV == "DEV":
    ...
    def webhook():
        bot.remove_webhook()
        bot.set_webhook(url='https://mia-test-bot.herokuapp.com/'+telegram_key)
        return 'Chat with the Bot  <a href ="https://t.me/miakurtzbot">here</a> \
          or   Check the project code <a href ="https://github.com/Mr-mittens/mia-test-bot">here</a>', 200

    if __name__ == "__main__":
        if datetime.datetime.now().strftime("%H:%M") in ["06:00","06:01","07:00","07:01"]:
            threading.Thread(target=scheduler).start()
